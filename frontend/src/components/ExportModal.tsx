      ...
      if (category === "report") {
        ...
        if (format === "json") {
          ...
          const contentHtml = `
            ...
            ${(exportData.mood_entries || []).slice(0, 10).map((m: any) => `
              <div class="card">
        ...
        if (format === "json") exportDataAsJSON(timeline, "sera_timeline");
          ...
          const html = timeline.map((item: any) => `
            <div class="card">
        ...
        if (format === "json") exportDataAsJSON({ phq9: phq, gad7: gad }, "sera_assessments");
          ...
          const html = `
            ...
            ${phq.map((p: any) => `<div class="card"><strong>Score ${p.score}/27</strong> - ${p.risk_category} (${p.created_at})</div>`).join('')}
            <h2>GAD-7 Anxiety History</h2>
            ${gad.map((g: any) => `<div class="card"><strong>Score ${g.score}/21</strong> - ${g.anxiety_level} (${g.created_at})</div>`).join('')}
          `;
